...
- **当前状态**：W0 进行中——工程骨架、数据层、组件库、项目看板、几何内核、3D 视图桥、调研流水线、**看图 → build123d 代码建模**（出规格 → 生成 → 自动修复 → 参数面板 / 指令修补 → 看图复核）已落地；进度见 [W0 状态](docs/w0-status.md)，路线见 [路线图](docs/06-roadmap.md)。
