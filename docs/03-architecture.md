...
| 成本定价器（schema v5） | `cost_models`：`params_json`（一整份 `CostParams`）· `unit_cost` · `chosen_price`（分）；`print_runs`：预估 / 实际的分钟与克数 · `result`(success/failed) · `fail_reason` · `deleted_at`（v5 新增）；`printers` / `materials`（v1 就有）；`library_settings(key, value_json)`（v5 新增，现在只有 `cost_defaults`） | [ADR-0007](adr/0007-cost-pricing.md)：**算钱的公式全在 `pp_common::cost`（纯函数，前后端共用）**，前端改参数即时重算、后端只存取；成本模型里存的是数、不是对档案的引用；一次成功率同时摊到材料和机时上 |
