...
└─ 设置                二级菜单,一次只显示一组:通用(外观 / 语言 / 演示模式)· 接口密钥 · 图片生成 · 资料库 · 关于与更新
                       (规划中的:渠道 · 预算 · 违禁词库;打印机与耗材、成本参数在「定价」里,品牌语气在「上架」里)
```
...
- **图不一定是生成的**([ADR-0011](adr/0011-import-and-drop.md)):自己的图(实拍、草图、找来的参考)可以导入——胶片条最前面的「导入」格(没图时是大图区中间的按钮,可多选)、**直接拖进窗口**、或者 `Ctrl+V`(截图、浏览器里「复制图片」、资源管理器里复制的图片文件)。导入的图和生成的图同等地位:能选中、采用、导出、送去建模,也能让 AI 接着改。**落点决定去向**:拖到画板这边 = 导入成画板上的一张图;拖到对话栏(或者在输入框里粘贴)= 只贴到这句话上当参考。悬着的时候两块地方各有一个虚线框说明松手之后会发生什么。一批里有坏图、HEIC、不是图片的文件时,能导入的照常导入,其余逐个说明原因。
