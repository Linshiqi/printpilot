...
| B-rep 为准 | STEP 是源头真值；网格由执行器自己按**绝对**弦差现导（0.02 mm / 0.2 rad，远高于 FDM 的精度），不存在「模型糊」的问题。不用 build123d 自带的 `export_stl`：它用相对偏差，大曲面没有误差上限（见「性能与质量复查」） |
...
2. **进程隔离**：独立子进程（`python -I -B`），临时工作目录，最小化环境变量（**不含任何接口密钥**），超时即杀。
   引擎进程是**常驻**的（`runner.py --serve`，省掉每次约 4 秒的冷启动），隔离措施与单次执行完全相同，另外保证任务之间不串味：每个任务全新的命名空间；AST 层**禁止给属性赋值**（`Box.x = …`、`math.pi = 3` 这类对共享对象的改动会留到下一个任务）；`math` 每任务发一份替身；跑满 40 个任务换新进程；超时 / 取消直接杀进程——这两种换新都在**后台线程**里做（`pp_cad::pool::WorkerPool`：任务的结果先返回，下一次建模不用等那 4 秒；换引擎包期间暂停）；stdin 一关（应用退出）进程自己退出，另有进程内看门狗兜底。常驻进程起不来时自动退回单次执行。
...
- **跑通 ≠ 合格**：恰好 1 个实体、B-rep 有效、贴床（|z_min| ≤ 0.05）、按规格首次生成时还要有一个平的底面（贴床的平面面积 ≥ 1 mm²）、包围盒与规格相差不超过 max(1 mm, 5%)（高度必须对上，X / Y 允许互换）、放得进成型空间、PARAMS 可解析且当前值在自己声明的范围内。
...
### 性能与质量复查（2026-09-21，v0.9.4 之后）

逐段给执行器计时（常驻进程里，同一段代码跑 3 遍取后 2 遍），发现时间并不花在「建模」上：

| 发现 | 实测 | 处理 |
|------|------|------|
| build123d 默认的精确包围盒在带圆角的零件上很慢 | 托盘（35 个面）一次 150~190 ms，比建出这个零件还久。慢的只有两类面：**环面**（圆角拐弯处，约 26 ms / 面）和**样条面**（约 19 ms / 面）；平面 / 圆柱 / 圆锥 / 球有解析解，约 0.05 ms / 面 | 混合包围盒（`runner.py::bounds`）：解析面照旧精确，环面和自由曲面用三角网顶点（只会偏小、不超过弦差 0.02 mm）。「快但不精确」的那种不能用：圆角处偏大约 1 mm，样条花瓶偏大 50 mm |
| `is_valid` 的兼容写法把属性求值了两遍 | 每遍都是一整遍 `BRepCheck`：165 孔的板 2 × 90 ms | 直接调一次 `BRepCheck_Analyzer` |
| `export_stl` 用的是**相对**偏差 | 小孔被过度细分（165 孔的板 84k 三角面、三角化 1.8 s），大曲面反而没有误差上限（60 mm 的放样花瓶只有 3.8k 面、弦差约 0.6 mm，打出来看得见棱） | 执行器自己三角化一次：**绝对**弦差 0.02 mm / 0.2 rad，量包围盒和写 STL 共用这一份网格 |
| 循环里做布尔 | 165 个孔：`for h in holes: body = body - h` 4.4 s，`body - holes` 0.18 s，结果完全一样 | 提示词加了硬规则，完整示例改成列表写法，`test_cheatsheet.py` 守着「示例里不许有循环」；超时的修复提示点名这个原因 |
| 进程被杀之后，下一次建模要等 4 秒 | 取消 / 超时 / 跑满 40 个任务之后槽位是空的，下一个任务同步起进程 | `pp_cad::pool::WorkerPool`：结果先返回，新进程在后台线程里起 |

整轮执行（校验 + 执行 + 量测 + 导出 STEP 和 STL）前后对比：

| 零件 | v0.9.4 | 现在 | STL 三角面 |
|------|--------|------|------------|
| 三槽线缆夹 | 67 ms | 57 ms | 68 → 68 |
| 带圆角的托盘 | 287 ms | **87 ms** | 17,948 → 4,892 |
| 旋钮（圆角 + 12 道防滑槽） | 352 ms | 259 ms | 13,908 → 8,330 |
| 放样花瓶 | 75 ms | 456 ms | 3,808 → 24,340（以前的太糙：弦差约 0.6 mm → 0.02 mm） |
| 165 孔的板（一次减一批） | 2,729 ms | 1,609 ms | 84,344 → 42,508 |
| 165 孔的板（循环里一个个减） | 8,000 ms | 5,882 ms | 同上 |

质量方面加了三样**不花模型调用**的东西：

- **错误提示表**（`CadScriptError::hint`）：build123d / OpenCascade 的原始报错经常说不到点子上——「选择器什么都没选中」和「对空的边列表做圆角」报出来都是一句 `IndexError: list index out of range`。13 种常见失败的签名（都是在真引擎上跑出来的，不是凭印象写的）各配一句修法，跟在原始错误后面喂给修复循环；认不出来的不加。
- **可打印性量测**：贴床的平面面积、离开床面且正朝下的平面面积（只看平面：精确、便宜；曲面悬垂要采样，先不做）。**没有平的底面**（球、躺着的圆柱）在按规格首次生成时算一条问题（`CadProblem::NoFlatBase`）；对话修改时不拦——用户要一个球就给他一个球——但指标卡上一直有提醒（`CadMetrics::print_notes`：没有平底 / 贴床面积太小建议加裙边 / 有多少悬空平面要搭桥或支撑）。
- 「贴床」的容差从 0.01 放到 0.05 mm：底部是环面 / 自由曲面的零件，最低点用三角网量，会偏高最多 0.02，不该被当成悬空（否则自动贴床之后还会再报一次，白白多问一轮模型）。
...
6. 要真实接口才能验证、所以还没动的三项（按预期收益排）：
   - **对话修改只交回改动的段**：现在每次修改，模型都重写整份脚本，而输出 token 是一轮耗时的大头（100 行的脚本约 1,500 token）。改成「只输出改了的 FEATURE 段」、由我们拼回去，预计一轮能快一半以上；风险是模型漏改相关的段——要用真模型量成功率。
   - **首次生成后自动看图复核**：现在要用户点。自动做能多抓一类「跑通了但不像」的错，代价是每次生成多一次视觉调用。
   - **规格尺寸逐项核对**：规格里每个特征的尺寸 vs PARAMS 里的值。特征尺寸的名字（`height`、`length`）在规格里有歧义，按名字硬对容易误报、白白多修一轮；要先看真模型实际怎么给参数命名。
5. ~~面板从预研页搬出来~~ **已完成**：建模成为一级入口「建模」，围绕和 AI 的持续对话来改模型，见 [ADR-0004](0004-modeling-studio.md)。
