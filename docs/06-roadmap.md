...
| 3 | 调研 Agent（流水线、流式报告、证据卡、机会卡、采用为项目、用户补充证据）；预览图（MiniMax 适配器、三种用途、提示词起草、采用与版本、AI 打标） | 提示词带回归样例 |
| 4 | 3D：**代码建模搬进项目的「模型」阶段**（W0 已在预研页跑通整条链路，见 ADR-0003）——版本挂项目、产品预览图直接作参考图、真模型实测并调提示词、引擎包下载（常驻引擎进程已在 W0 完成：改参数 0.2~0.3 秒）；导入自有模型；可打印性清单；调起切片软件；体积法估算。Tripo 适配器延后到有机造型需求出现时 | 预览与落盘分离；生成的代码只在沙箱里跑 |
