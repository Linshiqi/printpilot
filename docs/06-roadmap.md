...
| 周 | 交付 | 要点 |
|----|------|------|
| 2 | 数据层全量建表；项目看板（指针拖拽）；**项目中枢与阶段门已完成**（清单从产出里算出来，三个工作台挂到项目上，见 ADR-0006；完整的项目详情页等后四个阶段有东西可放时再做）；任务调度器（持久化、限速、预算闸门、重启恢复）；设置（密钥预设与测试连接、打印机 / 耗材、预算、代理、演示模式） | 演示模式从第一天就有——之后所有界面都能不花钱地开发与验收 |
...
| 5 | **成本定价器已完成**（一级入口「定价」：成本模型、三档建议价、每机时毛利、竞品价格带、产能、打样记录、档案与默认值，见 ADR-0007）；商品与笔记草稿生成；合规 Lint；发布包（电脑端 + 手机扫码）；回填链接；**打包版黄金路径验收** | 从扫码 / 打开到发布完成 ≤ 2 分钟实测 |
