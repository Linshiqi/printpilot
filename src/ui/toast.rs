...
pub fn Toasts(state: AppState) -> impl IntoView {
    view! {
        <Show when=move || state.toast.get().is_some()>
            <div class="fixed top-3 left-1/2 -translate-x-1/2 z-[200] max-w-[80vw] animate-toast-in">
                <div
                    class="flex items-start gap-2 bg-red-600 text-white text-xs px-4 py-2.5 rounded-lg shadow-lg selectable"
                    role="alert"
                >
        ...
        <Show when=move || state.info_toast.get().is_some()>
            <div class="fixed top-3 left-1/2 -translate-x-1/2 z-[200] max-w-[80vw] animate-toast-in">
                <div
                    class="flex items-start gap-2 bg-green-600 text-white text-xs px-4 py-2.5 rounded-lg shadow-lg"
                    role="status"
                >
